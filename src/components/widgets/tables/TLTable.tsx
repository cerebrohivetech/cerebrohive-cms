...
import DefaultImage from "../../../assets/Rectangle 23.svg"
import ActionMenu from "../../../assets/dots-menu.svg"

export function TestimonialsTable() {
    const data = [
        ["Client's Name", "Company", "Position", "Image", "action"],
                    ...
                    : <Row key={index}>
                        {row.map((item, index) => (
                            ...
                            <Data key={item} url={item}>
                                {item === ''? 
                                <Image src={DefaultImage} alt="profile" className="w-[48px] h-[48px] rounded-full"/>:
                                <Image src={item} alt="download" className="w-[24px] h-[24px]"/>
                                }   
                            </Data>:
                            index === 4?
                                <Data key={item} className="!justify-end">
